...
nebhaq gur ubhfr, fur fvgt nebhaq gur ubhfr.
...
and finally the Gothons blow up the ship from their ship and you die.
END
...
you arm and don't want to set it off.
END
...
it and then carefully place the bomb on the floor, pointing your blaster
at it.  You then jump back through the door, punch the close button and 
blast the lock so the Gothons can't get out.  Now that the bomb is placed 
...
EscapePod_bad = <<END
You jump into the pod and hit the eject button.  The pod escapes out into 
the void of space, then implodes as the hull ruptures, crushing your body
into jam jelly.
END
...
class Game
	...
	def central_corridor()
		puts CentralCorridor_

		prompt()
		action = gets.chomp()

		if action == "shoot!"
			puts CentralCorridor_shoot
			return :death

		elsif action == "dodge!"
			puts CentralCorridor_dodge
			return :death

		elsif action == "tell a joke"
			puts CentralCorridor_joke
	...
	def the_bridge()
		...
		if action == "throw the bomb"
			puts TheBridge_throw
			return :death

		elsif action == "slowly place the bomb"
			puts TheBridge_place
			return :escape_pod
		else
			puts NotCompute
			return :the_bridge
	...
	def escape_pod()
		...
		print "[pod #]> "
		guess = gets.chomp()
		if guess.to_i == 42 then guess = good_pod end
